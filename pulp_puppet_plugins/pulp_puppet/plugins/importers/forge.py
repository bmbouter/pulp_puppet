...
class SynchronizeWithPuppetForge(object):
    ...
    def __call__(self):
        ...
        """
        msg = _('Beginning sync for repository <%s>') % self.repo.id
        _logger.info(msg)
    ...
    def _parse_metadata(self):
        ...
        """
        msg = _('Beginning metadata retrieval for repository <%s>') % self.repo.id
        _logger.info(msg)
        ...
        try:
            ...
            if self._canceled:
                msg = _('Exception occurred on canceled metadata download: %s') % e
                _logger.warn(msg)
                self.progress_report.metadata_state = STATE_CANCELED
                return None
            msg = _('Exception while retrieving metadata for repository <%s>') % self.repo.id
        ...
        try:
            ...
            msg = _('Exception parsing metadata for repository <%s>') % self.repo.id
            _logger.exception(msg)
            self.progress_report.metadata_state = STATE_FAILED
    ...
    def _import_modules(self, metadata):
        ...
        """
        msg = _('Retrieving modules for repository <%s>') % self.repo.id
        _logger.info(msg)
        ...
        try:
            ...
            msg = _('Exception importing modules for repository <%s>') % self.repo.id
            _logger.exception(msg)
