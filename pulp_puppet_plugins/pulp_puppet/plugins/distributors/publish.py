...
class PuppetModulePublishRun(object):
    ...
    def perform_publish(self):
        ...
        """
        msg = _('Beginning publish for repository <%s>') % self.repo.repo_id
        _logger.info(msg)
    ...
    def _modules_step(self):
        ...
        try:
            ...
            msg = _('Exception during modules step for repository <%s>') % self.repo.repo_id
    ...
    def _metadata_step(self, modules):
        ...
        try:
            ...
            msg = _('Exception during metadata generation step for repository <%s>') % \
                  self.repo.repo_id
            _logger.exception(msg)
    ...
    def _init_build_dir(self):
        ...
        """
        msg = _('Initializing build directory for repository <%s>') % self.repo.repo_id
        _logger.info(msg)
    ...
    def _cleanup_build_dir(self):
        ...
        """
        msg = _('Cleaning up build directory for repository <%s>') % self.repo.repo_id
        _logger.info(msg)
    ...
    def _symlink_modules(self, modules):
        ...
        :type modules: list of pulp_puppet.plugins.db.models.Module
        """
        msg = _('Creating symlinks for modules in repository <%s>') % self.repo.repo_id
        _logger.info(msg)
    ...
    def _generate_metadata(self, modules):
        ...
        :type modules: list of pulp_puppet.plugins.db.models.Module
        """
        msg = _('Generating metadata for repository <%s>') % self.repo.repo_id
        _logger.info(msg)
    ...
    def _generate_dependency_data(self, modules):
        ...
        msg = _('generating dependency metadata in file %s') % filename
        _logger.debug(msg)
        # opens a new file for writing and overwrites any existing file
        db = gdbm.open(filename, 'n')
        try:
            for module in modules:
                path = os.path.join(self._repo_path, self._build_relative_path(module))  # TODO check that this path is right
